...
def handle_menu_commands(message, message_object, thread_id, thread_type, author_id, bot):
    command_names = "".join([
        ...
        f"➜ 📢 Dịch ngôn ngữ ({bot.prefix}dich)\n",
        f"➜ ⛅ Thời tiết ({bot.prefix}weather)\n",
        f"➜ 🤭 Thả thính ({bot.prefix}love)\n",
        f"➜ 🗯 Chat AI ({bot.prefix}chat)\n",
        f"➜ 🩴 Zalo ({bot.prefix}zl)\n",
        f"➜ 🎮 Game ({bot.prefix}bc {bot.prefix}tx)\n",
        f"➜ 🍼 Tính năng khác ({bot.prefix}or)\n",
        f"➜ 🙀 Đăng ký Hotmail ({bot.prefix}reghotmail)\n",
        f"➜ 💌 Chia sẻ code ({bot.prefix}share)\n",
        f"➜ 👩‍💼 Ảnh gái xinh ({bot.prefix}girl)\n",
        f"➜ 🤜 Xem thông tin Free Fire ({bot.prefix}ff)\n",
        f"➜ 😹 Xem độ Free Fire ({bot.prefix}doff)\n",
        f"➜ 🤖 Quản lý Bot ({bot.prefix}mybot)\n",
        f"➜ 🗞️ Tin tức ({bot.prefix}news)\n",
        f"➜ 🧨 Spam tin nhắn ({bot.prefix}war)\n",
        f"➜ 💬 Nội dung văn bản ({bot.prefix}text)\n",
        f"➜ 📀 Video gái xinh ({bot.prefix}vdgirl)\n",
        f"➜ ☁️ SoundCloud ({bot.prefix}scl)\n",
        f"➜ 🐳 Tính năng ẩn ({bot.prefix}hiden)\n",
        f"➜ 👊 Tấn công web ({bot.prefix}attack)\n",
        f"➜ 📔 Lưu link video vào file ({bot.prefix}save)\n",
        f"➜ 💍 Spam kết bạn Free Fire ({bot.prefix}spamff)\n",
...
def generate_menu_image(self, author_id, thread_id, thread_type):
    ...
    try:
        ...
        text_colors = [
            ...
            last_lines_color,
            last_lines_color,
            last_lines_color  # Thêm cho dòng thứ 6
        ]
        ...
        text_fonts = [
            ...
            font_text_bot,
            font_text_small,
            font_text_small  # Thêm cho dòng thứ 6
        ]
