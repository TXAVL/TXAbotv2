...
def generate_menu_image(bot, author_id, thread_id, thread_type):
    ...
    try:
        ...
        text_lines = [
            ...
            f"💞 Chào bạn, mình có thể giúp gì cho bạn nào? 😎",
            "😁 Bot luôn sẵn sàng phục vụ bạn! 🖤",
            f"🤖 Bot: {bot.me_name} 💻 Phiên bản: {bot.version} 📅 Cập nhật: {bot.date_update}",
            "Được tạo bởi TXA! 🌟"
        ]
        ...
        for i, line in enumerate(text_lines):
            ...
            if total_width > max_width:
                ...
                try:
                    ...
                    print(f"❌ Lỗi điều chỉnh kích thước font: {e}")
...
def qr(message, message_object, thread_id, thread_type, author_id, client):
    try:
        ...
        if message_text.startswith(command_prefix):
            ...
            if not content:
                content = "😍 Hãy kết bạn với mình nhé! 😊"
        else:
            content = "😍 Hãy kết bạn với mình nhé! 😊"
        ...
        message_text = f"🚦 {get_user_name_by_id(client, author_id)}, mã QR của {user_name} đây nhé! ✅"
...
def info(message, message_object, thread_id, thread_type, author_id, client):
    try:
        ...
        info_boxes = [
            ...
            {"title": "Sinh nhật", "value": datetime.fromtimestamp(user.dob).strftime("%d/%m/%Y") if user.dob else "Ẩn", "position": (left_column_x, start_y + vertical_spacing)},
            ...
            {"title": "Số điện thoại", "value": "Ẩn", "position": (right_column_x, start_y + vertical_spacing*3)},
        ]
        ...
        user_info = client.fetchUserInfo(target_id)
        user_name = user_info.changed_profiles.get(target_id).displayName
        message_info = f"🚦 {get_user_name_by_id(client, author_id)}, hồ sơ của {user_name} đây nhé! ✅"
        client.sendLocalImage(
            imagePath=image_path,
            thread_id=thread_id,
            thread_type=thread_type,
...
def get_id(bot, message_object, thread_id, author_id):
    try:
        ...
        return "⚠️ Đã xảy ra lỗi khi lấy thông tin! 😓"
...
def handle_zl_off(bot, thread_id):
    ...
    if "zl" in settings and thread_id in settings["zl"]:
        ...
        write_settings(bot.uid, settings)
        return f"🚦 Lệnh {bot.prefix}zl đã tắt ⭕ trong nhóm này! ✅"
    return "🚦 Nhóm chưa có thông tin cấu hình zl để ⭕ tắt! 😊"
    ...
    try:
        ...
        if len(content) < 2:
            msg = "".join([
                ...
                f"🏷️ Xem mã QR ({client.prefix}zl qr hoặc {client.prefix}zl qr @tag)\n",
                f"🆔 Xem UID Zalo ({client.prefix}zl id)\n",
                f"🧑‍💻 Xem hồ sơ Zalo ({client.prefix}zl info/@tag)\n"
            ])
