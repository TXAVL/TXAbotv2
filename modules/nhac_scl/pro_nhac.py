...
def generate_menu_image(self, author_id, thread_id, thread_type):
    ...
    try:
        ...
        text_lines = [
            f"Chào, {greeting_name} thân yêu! 🎉",
            f"💞 Anh đây, sẵn sàng hỗ trợ em nào! 😎",
            f"{self.prefix}scl on/off: 🚀 Bật/tắt tính năng nhạc",
            "😁 Sẵn sàng phục vụ em mọi lúc mọi nơi! 🖤",
            f"🤖 Bot: {self.me_name} 💻 Phiên bản: {self.version} 📅 Cập nhật: {self.date_update}",
            "Được tạo bởi TXA! 🌟"
        ]
...
def get_username(link):
    try:
        ...
        response = requests.get(api_url, headers=get_headers())
        data = response.json()
        return data.get('user', {}).get('username', 'Không rõ')
    except:
        return 'Không rõ'
...
def save_file_to_cache(url, filename):
    try:
        if not filename:
            print("❌ Lỗi trong save_file_to_cache: Tên file trống")
            return None

        print(f"📥 Tên file đầu vào: {filename}")

        safe_filename = re.sub(r'[<>:"/\\|?*]', '_', filename)
        safe_filename = safe_filename.replace(' ', '_').strip('_')
        
        if not safe_filename:
            print("❌ Lỗi trong save_file_to_cache: Tên file an toàn trống sau khi xử lý")
            return None
        ...
        print(f"📁 Tên file an toàn: {safe_filename}")
        ...
        print(f"❌ Lỗi HTTP trong save_file_to_cache: {http_err}")
        return None
    except requests.exceptions.RequestException as req_err:
        print(f"❌ Lỗi mạng trong save_file_to_cache: {req_err}")
        return None
    except OSError as os_err:
        print(f"❌ Lỗi hệ thống file trong save_file_to_cache: {os_err}")
        return None
    except Exception as e:
        print(f"❌ Lỗi bất ngờ trong save_file_to_cache: {e}")
        return None
...
def create_song_list_image(songs):
    try:
        ...
        if not background_images:
            print("❌ Không có ảnh trong thư mục background/")
        ...
        print(f"❌ Lỗi trong create_song_list_image: {e}")
        return None
    ...
    try:
        ...
        if os.path.exists(scl_path):
            try:
                ...
                print(f"❌ Lỗi tải hoặc xử lý sclden.jpg: {e}")
        else:
            print(f"❌ Không tìm thấy sclden.jpg tại {scl_path}")
        ...
        text_y = draw_gradient_text_line(draw, "🔊 Chill cùng nhạc nào! 🎧💘", text_x, text_y, font, emoji_font)
        ...
        print(f"❌ Lỗi trong create_single_song_image: {e}")
        return None
...
def handle_nhac_command(message, message_object, thread_id, thread_type, author_id, client):
    ...
    reactions = ["😍", "❤️", "/-ok", "👋", "🫠", "/-strong", "😂", "😉", "🥳", "🤩", "Dạ", "Yêu", "Vâng", "Ok", "Oki", "UwU", "😃", "🥰", "/-ok", "/-heart"]

    if not message or not isinstance(message, str):
        print("❌ Tin nhắn nhận được không hợp lệ")
        return
    ...
    if len(content) == 2 and content[0].lower() == f"{client.prefix}scl" and content[1].isdigit():
        print(f"📝 Người dùng chọn bài với số: {content[1]}")

        if author_id not in user_states:
            print(f"❌ Không có user_states cho author_id: {author_id}")
            client.sendReaction(message_object, "NO STATE", thread_id, thread_type, reactionType=75)
            return

        state = user_states[author_id]
        if time.time() - state['time_of_search'] > SEARCH_TIMEOUT:
            print(f"⏰ Hết hạn SEARCH_TIMEOUT cho author_id: {author_id}")
            del user_states[author_id]
            text = f"🚦 {username}, thời gian phản hồi đã hết! Vui lòng nhập {client.prefix}scl <tên bài hát> khác để nghe nhạc nhé! 🎶"
        ...
        if selector_index < 0 or selector_index >= len(songs):
            print(f"❌ Số thứ tự không hợp lệ: {content[1]}")
        ...
        if is_valid_image_url(cover_url):
            try:
                ...
                print(f"❌ Lỗi xử lý ảnh thumbnail: {e}")

        selected_number = content[1]
        text = f"""
🚦 {username} đã chọn: {selected_number}
📩 Tên bài hát: {title}
☁️ Nguồn: SoundCloud
🔗 Link: {link}
⏳ Chờ chút để tải nhạc nhé... 🎧"""
        mention = Mention(author_id, offset=2, length=len(username)) if thread_type != ThreadType.USER else None
        msg = Message(text=text, mention=mention)
        ...
        if temp_cover_path and os.path.exists(temp_cover_path):
            try:
                ...
                print(f"✅ Đã gửi và xóa ảnh thumbnail tạm: {temp_cover_path}")
            except Exception as e:
                print(f"❌ Lỗi khi gửi ảnh thumbnail: {e}")
        ...
        if is_valid_image_url(cover_url):
            ...
            rotating_disc_url = None
            print(f"❌ URL thumbnail không hợp lệ cho bài: {title}")
        ...
        if not audio_url:
            print(f"❌ Không tải được âm thanh cho bài: {title}")
        ...
        if not file_path:
            print(f"❌ Lỗi lưu tệp âm thanh cho bài: {title}")
        ...
        if not upload_response:
            print(f"❌ Lỗi tải lên tệp âm thanh cho bài: {title}")
        ...
        del user_states[author_id]
        print(f"✅ Đã xóa user_states cho author_id: {author_id}")
        return

    if len(content) < 2:
        print("📝 Không có nội dung tìm kiếm, hiển thị menu gợi ý.")
        if not author_id:
            author_id = "Không Biết"
   ...
   ➜ Ví dụ: 💞 {client.prefix}scl Đừng Thương
"""
        ...
        if image_path and os.path.exists(image_path):
            try:
                ...
                print(f"✅ Đã gửi và xóa ảnh: {image_path}")
            except Exception as e:
                print(f"❌ Lỗi khi gửi ảnh menu: {e}")
    ...
    if not songs:
        text = f"🚦 {username}, không tìm thấy bài hát nào cả! 😔"
        mention = Mention(author_id, offset=2, length=len(username)) if thread_type != ThreadType.USER else None
        client.replyMessage(Message(text=text, mention=mention), message_object, thread_id, thread_type, ttl=60000)
        return
    ...
    if image_path:
        try:
            ...
            print(f"❌ Lỗi gửi ảnh danh sách: {e}")
